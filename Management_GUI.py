...
class InvestmentManagerApp:
    ...
    def create_industry_management_tab(self):
        ...
        ttk.Entry(
            add_industry_frame, textvariable=self.new_industry_name, width=40
        ).pack(anchor="w")

        ttk.Button(
            add_industry_frame, text="Add Industry", command=self.add_industry
        ).pack(anchor="w")
    ...
    def create_sub_industry_management_tab(self):
        ...
        self.industry_dropdown_for_sub = ttk.Combobox(
            add_sub_industry_frame,
            textvariable=self.selected_industry_for_sub,
            width=40,
        )
        self.industry_dropdown_for_sub.pack(anchor="w")

        ttk.Label(add_sub_industry_frame, text="New Sub-Industry Name:").pack(
            anchor="w"
        )
        ttk.Entry(
            add_sub_industry_frame, textvariable=self.new_sub_industry_name, width=42
        ).pack(anchor="w")

        ttk.Button(
            add_sub_industry_frame,
            text="Add Sub-Industry",
            command=self.add_sub_industry,
        ).pack(anchor="w")
    ...
    def create_stocks_management_tab(self):
        ...
        stock_code_entry = ttk.Entry(
            add_stock_frame, textvariable=self.stock_code, width=20
        )
        stock_code_entry.pack(anchor="w")
        stock_code_entry.bind(
            "<KeyRelease>", lambda e: self.stock_code.set(self.stock_code.get().upper())
        )

        ttk.Label(add_stock_frame, text="Industry:").pack(anchor="w")
        self.industry_dropdown = ttk.Combobox(
            add_stock_frame, textvariable=self.selected_industry, width=40
        )
        self.industry_dropdown.pack(anchor="w")
        self.industry_dropdown.bind(
            "<<ComboboxSelected>>", self.load_sub_industries_dropdown
        )

        ttk.Label(add_stock_frame, text="Sub-Industry:").pack(anchor="w")
        self.sub_industry_dropdown = ttk.Combobox(
            add_stock_frame, textvariable=self.selected_sub_industry, width=40
        )
        self.sub_industry_dropdown.pack(anchor="w")

        ttk.Label(add_stock_frame, text="RSI Comparison Market:").pack(anchor="w")
        self.rsi_market_dropdown = ttk.Combobox(
            add_stock_frame, textvariable=self.rsi_comparison_market, width=40
        )  # Define rsi_market_dropdown
        self.rsi_market_dropdown.pack(anchor="w")

        ttk.Label(add_stock_frame, text="RSI Comparison Sector:").pack(anchor="w")
        self.rsi_sector_dropdown = ttk.Combobox(
            add_stock_frame, textvariable=self.rsi_comparison_sector, width=40
        )
        self.rsi_sector_dropdown.pack(anchor="w")

        ttk.Checkbutton(
            add_stock_frame, text="Commodity", variable=self.is_commodity
        ).pack(anchor="w")

        ttk.Button(add_stock_frame, text="Add Stock", command=self.add_stock).pack(
            anchor="w"
        )
    ...
    def load_sub_industries_dropdown(self, event=None):
        ...
        industry = (
            session.query(Industry).filter_by(name=selected_industry_name).first()
        )
        if industry:
            sub_industries = (
                session.query(SubIndustry).filter_by(industry_id=industry.id).all()
            )
        ...
        try:
            ...
            if name:
                ...
                self.show_temp_message(
                    title="Success",
                    message=f"Industry '{name}' has been added!",
                    duration=2000,
                )
            ...
            if industry_name and sub_industry_name:
                ...
                if industry:
                    sub_industry = SubIndustry(
                        name=sub_industry_name, industry_id=industry.id
                    )
                    ...
                    self.show_temp_message(
                        title="Success",
                        message=f"{sub_industry_name} has been added!",
                        duration=1000,
                    )
                    ...
                    messagebox.showwarning(
                        "Input Error", "Selected industry does not exist."
                    )
            else:
                messagebox.showwarning(
                    "Input Error",
                    "Both industry and sub-industry names cannot be empty.",
                )
    ...
    def load_industries_dropdown_for_sub(self):
        industries = session.query(Industry).all()
        self.industry_dropdown_for_sub["values"] = [
    ...
    def load_sub_industries(self):
        ...
        for sub_industry in sub_industries:
            industry = (
                session.query(Industry).filter_by(id=sub_industry.industry_id).first()
            )
            self.sub_industry_listbox.insert(
                tk.END, f"{sub_industry.name} (Industry: {industry.name})"
            )
    ...
    def add_stock(self):
        ...
        for code in codes:
            try:
                self.show_temp_message(
                    "Searching", f"Searching Yahoo Finance for {code}", duration=2000
                )

                # Check if the ticker already exists in the database
                existing_ticker = (
                    session.query(Stock).filter(Stock.code.ilike(code)).first()
                )
                if existing_ticker:
                    messagebox.showerror(
                        "Duplicate Ticker",
                        f"The ticker '{code}' already exists in the database.",
                    )
                    continue
                ...
                if "shortName" not in stock_data.info:
                    messagebox.showerror(
                        "Invalid Ticker",
                        "The stock code entered does not exist on Yahoo Finance.",
                    )
                    continue
                ...
                if not messagebox.askyesno(
                    "Confirm Share Name",
                    f"Is the share name '{correct_share_name}' correct?",
                ):
                    return
                ...
                industry = session.query(Industry).filter_by(name=industry_name).first()
                sub_industry = (
                    session.query(SubIndustry)
                    .filter_by(name=sub_industry_name, industry_id=industry.id)
                    .first()
                )
                ...
                existing_ticker = (
                    session.query(TickerName).filter_by(ticker=code).first()
                )
                ...
                messagebox.showinfo(
                    "Stock Added",
                    f"The stock '{correct_share_name}' with ticker '{code}' has been added successfully.",
                )
                self.stock_code.set("")
    ...
    def create_settings_tab(self):
        ...
        ttk.Label(settings_frame, text="Database Name:").pack(
            anchor="w", padx=10, pady=5
        )
        ttk.Entry(settings_frame, textvariable=self.db_name).pack(
            anchor="w", padx=10, pady=5
        )

        ttk.Label(settings_frame, text="Username:").pack(anchor="w", padx=10, pady=5)
        ttk.Entry(settings_frame, textvariable=self.db_user).pack(
            anchor="w", padx=10, pady=5
        )
    ...
    def create_portfolio_tab(self):
        ...
        ttk.Label(portfolio_tab, text="Portfolio Name:").pack(
            anchor="w", padx=10, pady=5
        )
        self.portfolio_name_var = tk.StringVar()
        ttk.Entry(portfolio_tab, textvariable=self.portfolio_name_var).pack(
            anchor="w", padx=10, pady=5
        )

        ttk.Button(
            portfolio_tab, text="Create Portfolio", command=self.create_portfolio
        ).pack(anchor="w", padx=10, pady=10)

        # Widget to show existing portfolios like in the stock management tab
        self.portfolio_tree = ttk.Treeview(
            portfolio_tab, columns=("Name",), show="headings", height=8
        )
    ...
    def save_settings(self):
        ...
        messagebox.showinfo(
            "Settings Saved", "Database configuration has been updated."
        )
    ...
    def load_industries(self):
        ...
        industries = (
            session.query(Industry).order_by(Industry.name).all()
        )  # Order by name
    ...
    def load_stocks(self):
        ...
        non_commodities = (
            session.query(Stock).filter_by(commodity=False).order_by(Stock.code).all()
        )
        for stock in non_commodities:
            industry = session.query(Industry).filter_by(id=stock.industry_id).first()
            sub_industry = (
                session.query(SubIndustry).filter_by(id=stock.sub_industry_id).first()
            )
            self.stock_listbox.insert(
                tk.END,
                f"{stock.code} - {stock.share_name} (Industry: {industry.name}, Sub-Industry: {sub_industry.name})",
            )

        # Load commodities next, ordered by code
        commodities = (
            session.query(Stock).filter_by(commodity=True).order_by(Stock.code).all()
        )
        ...
        if selected_index:
            ...
            if industry:
                self.show_edit_popup(
                    "Edit Industry",
                    industry.name,
                    lambda new_name: self.update_industry(industry, new_name),
                )
    ...
    def edit_sub_industry_popup(self, event):
        ...
        if selected_index:
            selected_name = self.sub_industry_listbox.get(selected_index).split(" (")[
                0
            ]  # Extract just the sub-industry name
            sub_industry = (
                session.query(SubIndustry).filter_by(name=selected_name).first()
            )
            if sub_industry:
                self.show_edit_popup(
                    "Edit Sub-Industry",
                    sub_industry.name,
                    lambda new_name: self.update_sub_industry(sub_industry, new_name),
                )
    ...
    def edit_stock_popup(self, event):
        ...
        if selected_index:
            selected_name = self.stock_listbox.get(selected_index).split(" - ")[
                0
            ]  # Extract just the stock code
            ...
            if stock:
                ...
                market_dropdown = ttk.Combobox(
                    edit_popup, textvariable=new_market_var, width=48
                )
                markets = (
                    session.query(RSIComparisonMarket)
                    .order_by(RSIComparisonMarket.name)
                    .all()
                )
                ...
                sector_dropdown = ttk.Combobox(
                    edit_popup, textvariable=new_sector_var, width=48
                )
                sectors = (
                    session.query(RSIComparisonSector)
                    .order_by(RSIComparisonSector.name)
                    .all()
                )
                ...
                ttk.Label(edit_popup, text="Portfolios:").pack(
                    anchor="w", padx=10, pady=5
                )
                ...
                for p in portfolios:
                    ...
                    ttk.Checkbutton(edit_popup, text=p.name, variable=var).pack(
                ...
                def save_stock_changes():
                    ...
                    new_portfolios = [
                        p for p, var in portfolio_vars.items() if var.get() == 1
                    ]
                ...
                tk.Button(edit_popup, text="Save", command=save_stock_changes).pack(
                    pady=10
                )
    ...
    def load_rsi_comparison_sectors(self):
        sectors = (
            session.query(RSIComparisonSector).order_by(RSIComparisonSector.name).all()
        )
    ...
    def create_rsi_comparison_sectors_tab(self):
        ...
        add_rsi_sector_frame.pack(
            side="right", fill="both", expand=True, padx=10, pady=10
        )

        self.new_rsi_sector_name = tk.StringVar()
        ttk.Label(add_rsi_sector_frame, text="New RSI Comparison Sector:").pack(
            anchor="w"
        )
        ttk.Entry(add_rsi_sector_frame, textvariable=self.new_rsi_sector_name).pack(
            anchor="w"
        )

        ttk.Button(
            add_rsi_sector_frame, text="Add Sector", command=self.add_rsi_sector
        ).pack(anchor="w")

        ttk.Button(
            add_rsi_sector_frame,
            text="Remove Selected Sector",
            command=self.remove_selected_rsi_sector,
        ).pack(anchor="w")
    ...
    def add_rsi_sector(self):
        ...
        if name:
            ...
            if (
                "shortName" not in sector_data.info
                or sector_data.info["longBusinessSummary"] is None
            ):
                messagebox.showerror(
                    "Invalid Sector Ticker",
                    "The RSI Comparison Sector code entered does not exist on Yahoo Finance.",
                )
                return
            ...
            try:
                ...
                self.show_temp_message(
                    title="Success", message=f"{name} has been added!", duration=1000
                )
            except IntegrityError:
                session.rollback()
                messagebox.showerror(
                    "Duplicate Sector", f"The sector '{name}' already exists."
                )
        else:
            messagebox.showwarning(
                "Input Error", "RSI Comparison Sector name cannot be empty."
            )
    ...
    def remove_selected_rsi_sector(self):
        ...
        if selected_index:
            ...
            sector = (
                session.query(RSIComparisonSector).filter_by(name=selected_name).first()
            )
            if sector:
    ...
    def create_rsi_comparison_markets_tab(self):
        ...
        add_rsi_market_frame.pack(
            side="right", fill="both", expand=True, padx=10, pady=10
        )

        self.new_rsi_market_name = tk.StringVar()
        ttk.Label(add_rsi_market_frame, text="New RSI Comparison Market:").pack(
            anchor="w"
        )
        ttk.Entry(add_rsi_market_frame, textvariable=self.new_rsi_market_name).pack(
            anchor="w"
        )

        ttk.Button(
            add_rsi_market_frame, text="Add Market", command=self.add_rsi_market
        ).pack(anchor="w")
        ttk.Button(
            add_rsi_market_frame,
            text="Remove Selected Market",
            command=self.remove_selected_rsi_market,
        ).pack(anchor="w")
    ...
    def add_rsi_market(self):
        ...
        if name:
            ...
            if (
                "shortName" not in market_data.info
                or market_data.info["underlyingSymbol"] is None
            ):
                messagebox.showerror(
                    "Invalid Market Ticker",
                    "The RSI Comparison Market code entered does not exist on Yahoo Finance.",
                )
                return
            ...
            try:
                ...
                session.rollback()
                messagebox.showerror(
                    "Duplicate Market", f"The market '{name}' already exists."
                )
        else:
            messagebox.showwarning(
                "Input Error", "RSI Comparison Market name cannot be empty."
            )
    ...
    def remove_selected_rsi_market(self):
        ...
        if selected_index:
            ...
            market = (
                session.query(RSIComparisonMarket).filter_by(name=selected_name).first()
            )
            if market:
    ...
    def load_rsi_comparison_markets(self):
        markets = (
            session.query(RSIComparisonMarket).order_by(RSIComparisonMarket.name).all()
        )

        # Update dropdown for RSI markets in stock management tab
        self.rsi_market_dropdown["values"] = [
            market.name for market in markets
        ]  # Use rsi_market_dropdown
        ...
        if selected_index:
            ...
            if market:
                ...
                def on_save(new_name):
                    ...
                    if (
                        "shortName" not in market_data.info
                        or market_data.info["longBusinessSummary"] is None
                    ):
                        messagebox.showerror(
                            "Invalid Market Ticker",
                            "The RSI Comparison Market code entered does not exist on Yahoo Finance.",
    ...
    def create_excel_upload_tab(self):
        ...
        ttk.Button(
            excel_frame, text="Select Excel File", command=self.select_excel_file
        ).pack(pady=20)
        ...
        ttk.Button(
            excel_frame, text="Upload Data", command=self.upload_excel_data
        ).pack(pady=20)
    ...
    def upload_excel_data(self):
        ...
        try:
            ...
            df = pd.read_excel(
                self.file_path, sheet_name="Summary"
            ...
            for index, row in df.iterrows():
                try:
                    vi_data = ViData(
                        code=truncate_string(
                            row["Code"], 10
                        ),  # Assuming 'Code' has a limit of 10 characters
                        eps=row.get("EPS", None),
                        nav=row.get("NAV", None),
                        sales=row.get("Sales", None),
                        eps_growth_f=truncate_string(
                            row.get("EPS_Growth_F", ""), 10
                        ),  # Adjust the length as needed
                        roe_f=row.get("ROE_F", None),
                        inst_profit_margin_f=row.get("Net_Profit_Margin_F", None),
                        sales_growth_f=truncate_string(
                            row.get("Sales_Growth_F", ""), 10
                        ),  # Adjust the length as needed
                        holding=truncate_string(
                            row.get("Holding", ""), 1
                        ),  # Assuming Holding is a single character
                        shares=row.get("Shares", None),
                        interest_cover=row.get("Interest_Cover", None),
                        comment=truncate_string(
                            row.get("Comment", ""), 50
                        ),  # Assuming 'Comment' has a 50-character limit
                        ...
                        act=row.get(
                            "ACT_HEPS", None
                        ),  # Assuming ACT_HEPS corresponds to the correct column
                        heps=row.get(
                            "ACT_HEPS", None
                        ),  # Assuming ACT_HEPS corresponds to the correct column
                        quality_rating=truncate_string(
                            row.get("Quality_Rating", ""), 10
                        ),  # Adjust the length as needed
                        div_decl=row.get("Div_Decl", None),
                        div_ldt=row.get(
                            "DIV_LDT", None
                        ),  # Should now be the correct date type
                        div_pay=row.get(
                            "DIV_Pay", None
                        ),  # Should now be the correct date type
                        rec=truncate_string(
                            row.get("REC", ""), 20
                        ),  # Assuming 'REC' has a 20-character limit
                        rec_on=truncate_string(
                            row.get("REC_on", ""), 20
                        ),  # Adjust the length as needed
                        ye_release=row.get(
                            "YE_release", None
            ...
            e_to_show = e.split("\n")[1] + "\n" + e.split("\n")[-1]
            messagebox.showerror(
                "Upload Error", f"An error occurred while uploading data: {e_to_show}"
            )
        except Exception as e:
            session.rollback()
