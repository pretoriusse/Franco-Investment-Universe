...
class AccuracyCallback(tf.keras.callbacks.Callback):  # type: ignore
    ...
    *target_accuracy* percent.
    ...
    def __init__(
        self, X_val, y_val, tolerance=0.10, target_accuracy=90.0, ticker: str = None
    ):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        logger.info(
            f"Validation Accuracy for {self.ticker} is within {self.tolerance * 100}%: {accuracy:.2f}%"
        )
        """
        ...
        if accuracy >= self.target_accuracy:
            logger.info(
                f"Target validation accuracy of {self.target_accuracy}% achieved. Stopping training."
            )
...
def plot_residuals(y_true, y_pred, ticker, model_dir):
    ...
    plots_dir = model_dir.replace("models", "plots")
    os.makedirs(plots_dir, exist_ok=True)
    file_path = os.path.join(plots_dir, f"residuals_{sanitized_ticker}.jpg")
...
def preprocess_data(
    hist, ticker: str = "", industry: str | None = None, sub_industry: str | None = None
):
    ...
    try:
        sentiment_df = db_queries.fetch_sentiment_for_ticker(
            ticker, days=len(hist) + 30
        )
    ...
    # Merge in macro / thematic sentiment for the themes relevant to this
    # ticker (MARKET, RAND, its commodity and sector). Missing days or themes
    # default to 0.0 (neutral). Feeding these macro *drivers* lets the model
    # learn cross-asset relationships (e.g. oil up -> fuel producers up).
    try:
        from assets.macro_sentiment import themes_for_stock

        themes = themes_for_stock(ticker, industry, sub_industry)
        macro_df = db_queries.fetch_macro_sentiment_history(themes, days=len(hist) + 30)
        if not macro_df.empty:
            macro_df["date"] = pd.to_datetime(macro_df["date"])
            hist = hist.merge(macro_df, on="date", how="left")
        for theme in themes:
            col = f"macro_{theme}"
        ...
        # Macro features are optional; never block training on their absence.
        pass
...
def build_bidirectional_gru_model(input_shape, hparams):
    ...
    model.add(
        Bidirectional(GRU(units=hparams["HP_GRU_UNITS"], kernel_regularizer=l2(0.001)))
    )
    model.add(Dropout(hparams["HP_DROPOUT"]))
...
def create_sequences(data, seq_length):
    ...
    if len(X.shape) == 2:
        logger.warning(
            f"Data for sequences has only 2 dimensions: {X.shape}. Reshaping to add the third dimension."
        )
...
def objective(trial, X_train, y_train, X_val, y_val, ticker):
    ...
    try:
        ...
        model = build_bidirectional_gru_model(
            (X_train.shape[1], X_train.shape[2]), current_hparams
        )
...
def optimize_hyperparameters(ticker, hparams, X_train, y_train, X_val, y_val):
    ...
    study = optuna.create_study(
        direction="minimize", study_name=f"Close params for: {sanitized_ticker}"
    )
    study.optimize(
        lambda trial: objective(trial, X_train, y_train, X_val, y_val, ticker=ticker),
        n_trials=hparams.get("OPTUNA_TRIALS", 50),
...
def train_new_model(
    ...
    accuracy_callback = AccuracyCallback(
        ...
        y_val,
        tolerance=0.10,
        target_accuracy=hparams.get("TARGET_ACCURACY", 90.0),
    ...
    logger.info(
        f"Model Performance for {sanitized_ticker}: MSE: {mse:.4f}, MAE: {mae:.4f}, R²: {r2:.4f}"
    )
...
def load_best_hyperparameters(ticker, model_dir):
    ...
    if os.path.exists(hyperparam_file):
        ...
        raise FileNotFoundError(
            f"Hyperparameters file not found for {ticker} at {hyperparam_file}"
        )


def check_and_train_model(
    ticker, hparams, seq_length=60, industry=None, sub_industry=None
):
    ...
    hist = hist.rename(
        columns={"low": "Low", "high": "High", "close": "close", "volume": "Volume"}
    )
    ...
    logger.info(
        f"Fetched {len(hist)} rows of data for {ticker} from {start_date} to {end_date}."
    )

    # Preprocess data
    hist = preprocess_data(
        hist, ticker=ticker, industry=industry, sub_industry=sub_industry
    )
    ...
    features = [
        ...
        "rolling_std_10",
        "sentiment_score",
    ]  # news-sentiment compound score in [-1, +1]
    ...
    # Append any macro/thematic sentiment columns added by preprocess_data so
    # the model sees the macro drivers (oil, gold, rand, sector, market...).
    features = features + sorted(c for c in hist.columns if c.startswith("macro_"))
    ...
    if X.shape[0] == 0 or y.shape[0] == 0:
        logger.error(
            f"Not enough data to create sequences for {ticker}. Skipping training."
        )
        return
    ...
    # A model saved with a different feature count cannot be fine-tuned (its
    # input-layer width is fixed); force a from-scratch retrain in that case.
    schema_changed = bool(
        model_metadata
        and model_metadata.get("feature_count") is not None
        and model_metadata.get("feature_count") != len(features)
    )
    if schema_changed:
        logger.info(
            f"Feature schema for {ticker} changed "
            f"({model_metadata.get('feature_count')} -> {len(features)}); retraining from scratch."
        )

    if (
        os.path.exists(model_path)
        and model_metadata
        and model_metadata.get("last_trained_date")
        ...
        if last_date_in_data <= last_trained_date and not RETRAIN:
            logger.info(
                f"No new data since last training on {last_trained_date}. Skipping training."
            )
        else:
            logger.info(
                f"New data available since last training on {last_trained_date}. Retraining model."
            )
            ...
            try:
                ...
                logger.warning(
                    f"No saved hyperparameters for {ticker}; fine-tuning with current defaults."
                )
            ...
            logger.info(
                f"Model retrained for {sanitized_ticker}. MSE: {mse:.4f}, MAE: {mae:.4f}, R²: {r2:.4f}"
            )
            del model
            clear_gpu_memory()
        ...
        try:
            ...
            if not os.path.exists(
                os.path.join(model_dir, "best_close_hyperparameters.json")
            ):
                best_hparams = optimize_hyperparameters(
                    ticker, hparams, X_train, y_train, X_val, y_val
                )
                clear_gpu_memory()
                ...
                with open(
                    file=os.path.join(model_dir, "best_close_hyperparameters.json"),
                    mode="r",
                ) as hparam_file:
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            logger.info(
                "Training interrupted by user. Clearing GPU memory and exiting."
            )
            clear_gpu_memory()
            sys.exit(0)
        except Exception as e:
    ...
    while True:
        try:
            ...
            logger.error(
                f"Encountered GPU error in scheduler loop: {e}. Restarting script..."
            )
            restart_script()
        except KeyboardInterrupt:
            logger.info(
                "Training scheduler stopped by user.\nClearing GPU memory and exiting."
            )
            clear_gpu_memory()
            sys.exit(0)
        except Exception as e:
