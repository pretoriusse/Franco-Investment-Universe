...
def plot_actual_vs_predicted(y_true, y_pred, ticker, model_dir):
    ...
    file_path = os.path.join(
        model_dir.replace("models", "plots"),
        f"adjusted_close_training_vs_actual_{hparams['HP_EPOCHS']}_epochs_{hparams['HP_DROPOUT']}_dropout_{hparams['HP_LSTM_UNITS']}_units.jpg",
    ).replace(".JO", "")
    print(file_path)
...
def check_and_train_model(ticker, hparams, seq_length=60):
    ...
    if (
        ...
        if last_date_in_data <= last_trained_date:
            ...
            logger.info(
                f"New data available. Retraining model with existing and new data."
            )
            ...
            os.makedirs(model_dir, exist_ok=True)
            with open(
                os.path.join(model_dir, "adjusted_close_metadata.json"), "w+"
            ) as f:
                json.dump(model_metadata, f)
            ...
            logger.info(
                f"Model retrained and saved for {sanitized_ticker}. MSE: {mse:.4f}, MAE: {mae:.4f}, R²: {r2:.4f}"
            )
            del model
