...
class AccuracyCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        logger.info(
            f"Validation Accuracy within {self.tolerance * 100}%: {accuracy:.2f}%"
        )
...
def preprocess_data(hist):
    ...
    hist["ATR"] = ta.volatility.average_true_range(
        hist["High"], hist["Low"], hist["Adj Close"], window=14
    )
...
def train_new_model(
    X_train, y_train, X_val, y_val, model_dir, model_path, hparams, sanitized_ticker
):
...
def check_and_train_model(ticker, hparams, seq_length=60):
    ...
    if (
        ...
        if not os.path.exists(os.path.join(model_dir, "best_hyperparameters.json")):
            ...
            with open(
                file=os.path.join(model_dir, "best_hyperparameters.json"), mode="r"
            ) as hparam_file:
...
def run_training_loop(hparams):
    ...
    for index, row in df.iterrows():
        if (
            "=" in row["code"]
            or row["commodity"]
            or row["code"] in ["%5EJ300.JO", "STXFIN.JO", "STXCAP.JO", "STXRES.JO"]
        ):
            continue
        try:
