...
def upgrade() -> None:
    ...
    op.alter_column("adj_runs", "share_name", existing_type=sa.VARCHAR(), nullable=True)
    op.alter_column("adj_runs", "industry", existing_type=sa.VARCHAR(), nullable=True)
    op.alter_column("adj_runs", "run_date", existing_type=sa.DATE(), nullable=True)
    ...
    op.alter_column("stocks", "industry_id", existing_type=sa.INTEGER(), nullable=False)
    op.alter_column(
        "stocks", "sub_industry_id", existing_type=sa.INTEGER(), nullable=False
    )
...
def downgrade() -> None:
    ...
    op.alter_column(
        "stocks", "sub_industry_id", existing_type=sa.INTEGER(), nullable=True
    )
    op.alter_column("stocks", "industry_id", existing_type=sa.INTEGER(), nullable=True)
    ...
    op.create_unique_constraint(
        "ukey_close_rundate", "close_runs", ["code", "run_date"]
    )
    op.drop_constraint(None, "adj_runs", type_="foreignkey")
    op.create_unique_constraint("ukey_for_data", "adj_runs", ["code", "run_date"])
    op.alter_column("adj_runs", "run_date", existing_type=sa.DATE(), nullable=False)
    op.alter_column("adj_runs", "industry", existing_type=sa.VARCHAR(), nullable=False)
    op.alter_column(
        "adj_runs", "share_name", existing_type=sa.VARCHAR(), nullable=False
    )
    op.create_table(
        "strsvp_Sheet1",
