...
from assets.database_queries import (
    fetch_latest_dividend_date,
    insert_dividends_batch,
...
def upload_dividends(ticker, progress: UploadProgress, max_retries=5, delay=2):
    ...
    while attempt < max_retries:
        try:
            ...
            print(
                f"An error occurred while processing dividend for ticker {ticker}: {e}. Retrying ({attempt + 1}/{max_retries})..."
            )
            attempt += 1
    ...
    if attempt == max_retries:
        print(
            f"Failed to process dividend for ticker {ticker} after {max_retries} attempts."
        )
...
def main():
    ...
    with ThreadPoolExecutor(
        max_workers=5
    ) as executor:  # Reduce max_workers to limit the number of simultaneous requests
        futures = []
        for index, row in stock_universe.iterrows():
            futures.append(executor.submit(upload_dividends, row["code"], progress))
            time.sleep(
                2
            )  # Add delay between each ticker start (2 seconds in this example)
