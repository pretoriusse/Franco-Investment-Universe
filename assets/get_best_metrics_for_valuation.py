...
def process_data(df):
    try:
        ...
        logger.info(
            f"Data after date filtering: {df.shape[0]} rows, {df.shape[1]} columns."
        )
        ...
        logger.info(
            f"Data after handling zeros and imputing missing values: {df.shape[0]} rows, {df.shape[1]} columns."
        )
        ...
        logger.info(
            f"Data after computing price movement: {df.shape[0]} rows, {df.shape[1]} columns."
        )
...
def define_time_windows(df, window_years=5, end_gap_years=2, step_months=1):
    ...
    while current_start >= earliest_start:
        ...
        if current_end <= end_date_limit:
            ...
            logger.debug(
                f"Window added: {current_start.date()} to {current_end.date()}"
            )
        else:
            logger.debug(
                f"Window skipped (end date beyond limit): {current_start.date()} to {current_end.date()}"
            )
        current_start -= step
...
def plot_best_pair(df, best_pair, best_model, y_column, plot_dir, window=None):
    try:
        ...
        plt.title(
            f"Actual vs Predicted Price Movement using {best_pair}"
...
def main():
    ...
    for window_start, window_end in windows:
        ...
        logger.info(
            f"Evaluating window: {window_start.date()} to {window_end.date()} with {window_df.shape[0]} records."
        )

        if window_df.empty:
            logger.warning(
                f"No data in window: {window_start.date()} to {window_end.date()}. Skipping."
            )
            continue

        # Evaluate feature pairs in the current window
        best_pair, best_r2, best_model, best_coefficients = (
            evaluate_feature_pairs_window(window_df, features, y_column)
        )

        logger.info(
            f"Window {window_start.date()} - {window_end.date()}: Best R2 = {best_r2:.4f} with pair {best_pair}"
        )
    ...
    if qualifying_windows:
        for idx, qw in enumerate(qualifying_windows, 1):
            ...
            print(
                f"Feature Pair: {qw['best_pair']} with R-squared: {qw['best_r2']:.4f}"
            )
            print(
                f"Time Period: {qw['window_start'].date()} to {qw['window_end'].date()}"
            )
            print(f"Coefficients: {qw['best_coefficients']}")

            # Define plot directory
            ticker = (
                df["ticker"].iloc[0].replace(".JO", "")
                if "ticker" in df.columns
                else "Unknown"
            )
            plot_dir = os.path.join(
                "plots", f"Qualifying_Window_{idx}", ticker, "metrics"
            )
            ...
            plot_best_pair(
                df[
                    (df["date"] >= qw["window_start"])
                    & (df["date"] <= qw["window_end"])
                ],
                qw["best_pair"],
                qw["best_model"],
                y_column,
        ...
        logger.error(
            "No qualifying feature pairs found across all time windows with R-squared >= 75%."
        )
