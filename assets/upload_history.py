...
except ImportError:  # older yfinance versions don't define this
...
def upload_ticker(
    ...
    for attempt in range(max_retries):
        try:
            ...
            logging.warning(
                f"Rate limit hit for {ticker}, sleeping {delay}s (attempt {attempt}/{max_retries})"
            )
            time.sleep(delay)
            delay *= 2
