...
def download_zar():
    ...
    if latest_date:
        latest_date = pd.Timestamp(
            latest_date
        )  # Convert latest_date to pandas Timestamp
        zar_data = zar_data[pd.to_datetime(zar_data["Date"]) > latest_date]
        logging.debug(
            f"Data filtered from the last database date {latest_date}: {zar_data.head()}"
        )
    ...
    if not batch:
        logging.error(
            "No valid records to insert into ZAR/USD after cleaning. Batch might contain invalid dates or missing data."
        )
        return

    logging.debug(
        f"Prepared batch for insertion into zar_usd: {batch[:5]}"
    )  # Log only the first 5 records for brevity
...
def update_zar_periods():
    ...
    for date, overbought_oversold in zar_data:
        ...
        try:
            ...
            logging.error(
                f"Invalid decimal operation for date {date} with value {overbought_oversold}"
            )
