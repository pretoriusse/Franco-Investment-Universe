...
from assets.models import TechnicalAnalysis, NewsSentiment, MacroSentiment
...
from webapp.models import Subscribers, PortfolioTracker
from assets.portfolio_signals import compute_verdict
...
def fetch_stock_universe_from_db() -> pd.DataFrame:
    ...
    try:
        ...
        stock_universe = pd.DataFrame(
            ...
            columns=[
                ...
                "industry",
                "sub_industry",
                "rsi_comparison_market",
                "rsi_comparison_sector",
                "commodity",
            ],
        )
...
def fetch_stock_and_commodity_universe_from_db() -> pd.DataFrame:
...
def fetch_commodity_universe_from_db() -> pd.DataFrame:
...
def get_ticker_from_db(ticker: str) -> pd.DataFrame:
    ...
    try:
        ticker_data = (
            ...
            .order_by(StockDataHistory.date.asc())
            .all()
        )
...
def get_ticker_from_db_with_date_select(
...
def get_commodities_from_db(ticker: str) -> pd.DataFrame:
    ...
    try:
        commodity_data = (
            ...
            .order_by(ShowCommodities.date.asc())
            .all()
        )
...
def fetch_latest_date_for_ticker(ticker: str) -> date | None:
    """Return the most recent stored trading date for *ticker*, or None."""
    session = Session()
    try:
        result = (
            session.query(func.max(StockDataHistory.date))
            .filter(StockDataHistory.ticker == ticker)
            .scalar()
        )
...
def insert_stock_data_history_batch(
    batch: list[dict[str, Any]], on_conflict_update: bool = False
) -> None:
        ...
        if on_conflict_update:
            ...
            upsert_stmt = insert_stmt.on_conflict_do_update(
                index_elements=[
                    "ticker",
                    "date",
                ],  # Specify the columns that define a conflict
            ...
            session.bulk_insert_mappings(StockDataHistory.__mapper__, batch)
...
def update_zar_periods() -> None:
    ...
    try:
        ...
        zar_data = (
            session.query(ZARUSD.date, ZARUSD.overbought_oversold)
            .filter(ZARUSD.overbought_oversold.isnot(None))
            .order_by(ZARUSD.date)
            .all()
        )

        current_period: list[Any] = []  # [start_date, end_date, 'good'|'bad']
        current_type = None
        ...
        for row_date, overbought_oversold in zar_data:
            try:
                ...
                logger.error(
                    f"Error processing overbought_oversold for date {row_date}: {e}"
                )
...
def insert_dividends_batch(batch: list[dict[str, Any]]) -> None:
...
def fetch_latest_commodity_date(ticker: str) -> date | None:
    """Return the most recent stored date for a commodity ticker, or None."""
    session = Session()
    try:
        result = (
            session.query(func.max(Commodity.date))
            .filter(Commodity.ticker == ticker)
            .scalar()
        )
        return result if result else None
    except Exception as e:
...
def insert_commodities_batch(
    data_list: list[dict[str, Any]], on_conflict_update: bool = False
) -> None:
...
def fetch_latest_date_for_zar(ticker: str) -> date | None:
...
def insert_zar_usd_batch(batch: list[dict[str, Any]]) -> None:
    ...
    try:
        ...
        cleaned_batch = [
            record for record in batch if record["date"] and pd.notnull(record["date"])
        ]

        # Check if the cleaned batch is empty after filtering
        if not cleaned_batch:
            logger.error(
                "No valid records to insert into ZAR/USD after cleaning. Batch might contain invalid dates."
            )
            return
...
def fetch_all_zar_usd() -> list[Any]:
...
def insert_zar_good_period(period: tuple[date, date, Any]) -> None:
    ...
    try:
        ...
        stmt = stmt.on_conflict_do_update(
            index_elements=["start_date"], set_={"end_date": stmt.excluded.end_date}
        )
        session.execute(stmt)
        session.commit()
    except Exception as e:
...
def insert_zar_bad_period(period: tuple[date, date, Any]) -> None:
...
def insert_prediction(
    date: date, code: str, adj_close: float | None = None, close: float | None = None
) -> None:
    ...
    try:
        ...
        session.execute(
            query, {"date": date, "code": code, "adj_close": adj_close, "close": close}
        )
        session.commit()
    except SQLAlchemyError as e:
...
def upload_adjusted_close(file_path: str) -> None:
    ...
    try:
        ...
        # NaN → None so unfilled cells land as SQL NULL, not numeric 'NaN'
        ...
        adj_runs_list = [
            AdjRuns(
                ...
                daily_tr_return=record.get("daily_tr_return"),
                daily_close_return=record.get("daily_close_return"),
                true_range=record.get("true_range"),
                true_range_pct=record.get("true_range_pct"),
                atr_14_pct=record.get("atr_14_pct"),
                vol_24d=record.get("vol_24d"),
                vol_55d=record.get("vol_55d"),
                vol_ratio_24_55=record.get("vol_ratio_24_55"),
                return_24d=record.get("return_24d"),
                return_55d=record.get("return_55d"),
                risk_adj_mom_24d=record.get("risk_adj_mom_24d"),
                drawdown_55d=record.get("drawdown_55d"),
                avg_volume_24d=record.get("avg_volume_24d"),
                volume_ratio_24d=record.get("volume_ratio_24d"),
                ma_24d=record.get("ma_24d"),
                ma_55d=record.get("ma_55d"),
                price_vs_ma_24d=record.get("price_vs_ma_24d"),
                price_vs_ma_55d=record.get("price_vs_ma_55d"),
                ma_trend_24_55=record.get("ma_trend_24_55"),
...
def upload_close_runs(file_path: str) -> None:
    ...
    try:
        ...
        # NaN → None so short-history metrics land as SQL NULL, not numeric 'NaN'
        df = df.astype(object).where(pd.notna(df), None)
...
def fetch_active_subscribers() -> list["Subscribers"]:
...
def update_subscriber(subscriber_id: int, update_data: dict[str, Any]) -> bool:
    """
...
def insert_technical_analysis_batch(batch: list[dict[str, Any]]) -> None:
...
def insert_sentiment_batch(batch: list[dict[str, Any]]) -> None:
...
def insert_macro_sentiment_batch(batch: list[dict[str, Any]]) -> None:
    """Upsert a list of daily macro-sentiment records keyed on (date, theme)."""
    session = Session()
    try:
        stmt = insert(MacroSentiment).values(batch)
        stmt = stmt.on_conflict_do_update(
            index_elements=["date", "theme"],
            set_={
                "sentiment_score": stmt.excluded.sentiment_score,
                "article_count": stmt.excluded.article_count,
                "positive_count": stmt.excluded.positive_count,
        ...
        logger.error(f"Error inserting macro sentiment batch: {e}")
        session.rollback()
    finally:
        session.close()


def fetch_macro_sentiment_history(themes: list[str], days: int = 60) -> pd.DataFrame:
    """Return daily macro-sentiment for *themes* over the last *days*, pivoted wide.

    The result has one row per date and one ``macro_<THEME>`` column per
    requested theme holding that theme's ``sentiment_score`` (missing themes
    are present as all-NaN columns so callers can ``fillna(0.0)`` and merge by
    date). Returns an empty DataFrame when no records exist.
    """
    ...
    try:
        ...
        rows = (
            session.query(
                MacroSentiment.date,
                MacroSentiment.theme,
                MacroSentiment.sentiment_score,
            )
            .filter(MacroSentiment.theme.in_(themes), MacroSentiment.date >= cutoff)
            .order_by(MacroSentiment.date.asc())
            .all()
        )
        df = pd.DataFrame(rows, columns=["date", "theme", "sentiment_score"])
        if df.empty:
            return df
        wide = df.pivot_table(
            index="date", columns="theme", values="sentiment_score", aggfunc="last"
        ).reindex(columns=themes)
        wide.columns = [f"macro_{col}" for col in wide.columns]
        return wide.reset_index()
    except Exception as e:
        logger.error(f"Error fetching macro sentiment history: {e}")
        return pd.DataFrame()
    finally:
        session.close()


def get_portfolio_with_signals(subscriber_id: int) -> list[dict[str, Any]]:
    """Return one subscriber's holdings joined with today's buy/sell verdict.

    Holdings live in the webapp DB; the signals used for the verdict live in
    the sharesdata ``adj_runs`` table, so the two are fetched separately and
    merged in Python (they are on different engines and can't be SQL-joined).
    """
    # 1. This user's holdings (webapp DB)
    wsession = WebApp_Session()
    try:
        holdings = (
            wsession.query(PortfolioTracker)
            .filter_by(subscriber_id=subscriber_id)
            .all()
        )
        holdings = [
            {"id": h.id, "ticker": h.ticker, "weight": h.weight, "comment": h.comment}
            for h in holdings
        ]
    except SQLAlchemyError as e:
        logger.error(f"Error fetching portfolio for subscriber {subscriber_id}: {e}")
        return []
    finally:
        wsession.close()
    ...
    # 2. Latest adj_runs row per ticker (sharesdata DB). One shared run_date/day.
    # ponytail: portfolio_tracker.ticker is assumed to equal stocks.code; a
    # mismatch just yields an UNKNOWN verdict, never a wrong one.
    tickers = [h["ticker"] for h in holdings]
    session = Session()
    signals: dict[str, AdjRuns] = {}
    try:
        latest_date = session.query(func.max(AdjRuns.run_date)).scalar()
        if latest_date is not None:
            rows = (
                session.query(AdjRuns)
                .filter(
                    AdjRuns.run_date == latest_date,
                    AdjRuns.code.in_(tickers),
                )
                .all()
            )
            signals = {r.code: r for r in rows}
    except SQLAlchemyError as e:
        logger.error(f"Error fetching signals for portfolio: {e}")
    finally:
        session.close()

    for h in holdings:
        r = signals.get(h["ticker"])
        if r is None:
            h["verdict"], h["reason"], h["current_price"] = (
                "UNKNOWN",
                "no market data",
                None,
            )
        ...
        verdict, reason = compute_verdict(
            ...
            next_month_prediction=r.next_month_prediction,
            ma24=r.ma24,
            ma55=r.ma55,
            z_score=r.z_score,
        )
        h["verdict"], h["reason"] = verdict, reason
        h["current_price"] = (
...
def get_all_latest_signals() -> list[dict[str, Any]]:
    """Return every ticker's latest adj_runs row plus its buy/sell verdict.

    Used by the CSV data API. One row per ticker for the most recent run_date.
    """
    session = Session()
    try:
        latest_date = session.query(func.max(AdjRuns.run_date)).scalar()
        if latest_date is None:
            return []
        rows = session.query(AdjRuns).filter(AdjRuns.run_date == latest_date).all()
    except SQLAlchemyError as e:
        ...
        out.append(
            {
                ...
                "current_price": r.current_price,
                "next_week_prediction": r.next_week_prediction,
                "next_month_prediction": r.next_month_prediction,
                "z_score": r.z_score,
                "ma24": r.ma24,
                "ma55": r.ma55,
