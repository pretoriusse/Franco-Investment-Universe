...
class UploadProgress:
    def __init__(self, total):
        self.total = total
        self.progress = tqdm(
            total=total, desc="Total Progress", unit="ticker", position=0
        )
        self.current_progresses = {}

    def update_ticker_progress(self, ticker, progress):
        if ticker not in self.current_progresses:
            self.current_progresses[ticker] = tqdm(
                total=100,
                desc=f"{ticker} Progress",
                unit="%",
                position=len(self.current_progresses) + 1,
                leave=True,
            )
...
def upload_ticker(
    ticker, comparison_market, comparison_sector, progress: UploadProgress
):
    max_retries = 3
        ...
        try:
            ...
            if latest_date:
                ...
                if check_date.date() > now.date():
                    logging.info(
                        f"No new data needed for ticker {ticker} as the latest date is today."
                    )
                ...
                logging.info(
                    f"Downloading data for {ticker} starting from {start_date}"
                )
            ...
            if df.empty:
                logging.error(
                    f"No data found for ticker {ticker}. It may be delisted or not available."
                )
                progress.update_ticker_progress(ticker, 100)
                return
            ...
            os.makedirs(os.path.join("data", f"{tickerName}"), exist_ok=True)
            ...
            if batch:
                insert_commodities_batch(batch, on_conflict_update=True)  # Use upsert
                progress.update_ticker_progress(ticker, 100)
            ...
            logging.error(f"{ticker} possibly delisted; {e}. Skipping...")
            progress.update_ticker_progress(ticker, 100)
            return
        except Exception as e:
            logging.error(
                f"An error occurred: {e} for ticker {ticker}. Attempt {attempt + 1}/{max_retries}"
            )
            time.sleep(delay)
            delay *= 2  # Exponential backoff
    else:
        logging.error(
            f"Failed to download data for {ticker} after {max_retries} attempts. Skipping..."
        )


def main():
    # Fetch stock universe from the database
    stock_universe = fetch_commodity_universe_from_db()
    ...
    for _, row in stock_universe.iterrows():
        if row["commodity"]:  # Skip commodities if needed
            upload_ticker(
                row["code"],
                row["rsi_comparison_market"],
                row["rsi_comparison_sector"],
                progress,
            )
            time.sleep(
                1 + random.uniform(0, 5)
            )  # Wait 1 to 5 seconds between downloads
