...
def plot_price_and_bollinger_bands_close(data, ticker):
    ...
    plt.ylim(min(data['close'].min(), data['Bollinger_Low'].min()), 
...
def process_ticker_close(ticker, commodity, name):
    ...
    logger.debug(Fore.LIGHTGREEN_EX + f"Creating close bollinger data for: {ticker}\n" + Fore.RESET)
...
def add_close_rsi_comparisons(df):
    ...
    for index, row in df.iterrows():
        try:
            ticker = row['code']
            logger.debug(Fore.LIGHTMAGENTA_EX + f"Generating Close RSI for: {ticker}" + Fore.RESET)
            ...
            for entry in top_bottom_data[metric][group]:
                ...
                if ticker not in added_tickers:
                    stock_img = {
                        ...
                        'ticker': ticker,
                        'prediction': encode_image_to_base64(f'plots/{ticker.replace(".JO", "")}/close_prediction.png'),
        ...
        try:
            ...
            if not DEBUGGING:
                try:
                    ...
                    logger.error(ex)
                    logger.error(Fore.RED + 'Close runs data already exists for today' + Fore.RESET)
            ...
            if SUMMARY_REPORT:
                summary_pdf_filename = os.path.join(reports_dir, f'{today}', 'close_summary.pdf')
            ...
            detailed_pdf_filename = os.path.join(reports_dir, f'{today}', 'close_detailed.pdf')
            ...
            for subscriber in subscribers:
                user_pdf_filename = os.path.join(reports_dir, f'{today}', f'user_{subscriber.id}_close_detailed.pdf')
