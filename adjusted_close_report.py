...
# the bollinger workers run on a thread pool, so pyplot sections hold this lock.
...
def process_image(img_path):
    ...
    Previously also wrote unused ``_resized.png`` / ``_compressed.jpg`` copies
    next to every chart; the reports embed the original PNG and nothing reads
    the copies, so for ~200 tickers × 4 charts that was ~1,600 wasted writes.
    """
    return img_path
...
def generate_pdf_with_password(
    input_html, output_pdf, user_password, owner_password="0306245353082"
):
...
def plot_price_and_bollinger_bands_adjusted_close(data, ticker):
    data = data.dropna(
        subset=["Bollinger_High", "Bollinger_Low", "Adj Close"]
    ...
    plt.ylim(
        min(data["Adj Close"].min(), data["Bollinger_Low"].min()),
...
def plot_stock_adjusted_close_last_two_years(
    unscaled_close, ticker, next_week_predictions, next_month_predictions, name
):
    logger.info(
        f"Starting adjusted close to plot stock data for the last year for ticker: {ticker}"
    )
...
def plot_model_vs_actual(
    ...
    logger.info(
        f"Starting adjusted close to plot model vs actual data with enhanced insights."
    )
...
def plot_volume_data_last_two_years(
    unscaled_volume,
    ticker,
    next_week_volume_predictions=[],
    next_month_volume_predictions=[],
    name="",
):
    logger.info(
        f"Starting adjusted close to plot volume data for the last year for ticker: {ticker}"
    )
    plt.figure(figsize=(16, 8))
...
def process_ticker_adjusted_close(ticker, commodity, name):
    ...
    logger.info(
        Fore.LIGHTGREEN_EX
        + f"Creating adjusted close bollinger data for: {ticker}\n"
        + Fore.RESET
    )
    ...
    # pyplot is global/not thread-safe; serialise drawing while the slow DB
    # read above is free to overlap across pool workers.
    with _plot_lock:
...
def train_new_model(X, y, model_dir, model_path, hparams, sanitized_ticker):
    ...
    # Fit the model
    history = model.fit(
        X, y, epochs=hparams["HP_EPOCHS"], batch_size=64, validation_split=0.1
    )
...
def predict_adjusted_close_value(hist, hparams, ticker):
    ...
    model_path = os.path.join(
        model_dir, f"{sanitized_ticker}_Adjusted_Close_Model.keras"
    )
    ...
    # never producing a cache hit. Load it, use it, and free it (clear_session
    # below) at the end of this call.
    if os.path.exists(model_path):
        model: SequentialType = load_model(model_path)
        logger.info(f"Loaded adj-close model for {ticker} from disk")
    else:
    ...
    # Free this ticker's model from (GPU) memory before moving on; otherwise
    # VRAM use grows linearly with the number of tickers.
    del model
    clear_session()
        ...
        try:
            ...
            if not PREDICTION:
                ...
                (
                    ...
                    next_week_predictions,
                    next_month_predictions,
                ) = predict_adjusted_close_value(hist, hparams, ticker)
                next_week_prediction, next_month_prediction, sentiment_score = (
            ...
            plot_stock_adjusted_close_last_two_years(
                unscaled_close,
                ticker.replace(".JO", ""),
                next_week_predictions,
                next_month_predictions,
                ...
                stock_images.append(
                    {
                        ...
                        "adj_prediction": encode_image(
                            process_image(
                                f"{graph_dir}/{ticker.replace('.JO', '')}/adj_close_prediction.png"
                            )
                        ),
                        "volume_prediction": encode_image(
                            process_image(
                                f"{graph_dir}/{ticker.replace('.JO', '')}/volume.png"
                            )
                        ),
                        "bollinger": encode_image(
                            process_image(
                                f"{graph_dir}/{ticker.replace('.JO', '')}/adj_bollinger.png"
                            )
                        ),
                        "overbought_oversold": encode_image(
                            process_image(
                                f"{graph_dir}/{ticker.replace('.JO', '')}/adj_overbought_oversold.png"
                            )
                        ),
                    }
                )
...
def generate_bollinger_and_overbought_oversold_adjusted_close():
    ...
    # Spread work over a small thread pool (one worker per core, capped) so the
    # slow per-ticker DB reads overlap, instead of spawning ~200 threads 1s apart.
    # Drawing itself is serialised via _plot_lock inside process_ticker_adjusted_close.
    max_workers = min(8, (os.cpu_count() or 4))
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(
                process_ticker_adjusted_close,
                row["code"],
                row["commodity"],
                row["share_name"],
            ): row["code"]
    ...
    for index, row in df.iterrows():
        try:
            ...
            logger.info(
                ...
                + f"Generating Adjusted Close RSI for: {ticker}"
                + Fore.RESET
            )
            ...
            rsi = calculate_rsi_adjusted_close(
            ...
            logger.error(
                Fore.RED + f"Error calculating Adjusted Close RSI for {ticker}:"
            )
            logger.error(ex)
            logger.info(Fore.RESET)
            df.at[index, "SECTOR RSI 1M"] = round(0, 2)
                ...
                if ticker not in added_tickers:
                    stock_img = {
                        ...
                        "prediction": encode_image_to_base64(
                            f"plots/{ticker.replace('.JO', '')}/adj_close_prediction.png"
                        ),
                        "bollinger": encode_image_to_base64(
                            f"plots/{ticker.replace('.JO', '')}/adj_bollinger.png"
                        ),
                        "overbought_oversold": encode_image_to_base64(
                            f"plots/{ticker.replace('.JO', '')}/adj_overbought_oversold.png"
                        ),
                    }
...
def create_detailed_pdf(
    ...
    if summary_report:
        ...
        data["Next_Week_Prediction_Change"] = round(
            data["Next Week Prediction"] * 100, 2
        )
        data["Next_Month_Prediction_Change"] = round(
            data["Next Month Prediction"] * 100, 2
        )
...
def daily_job():
    while True:
        try:
            ...
            stock_data, stock_images, total_value_next_week, total_value_next_month = (
                fetch_data(hparams)
            )
            ...
            stock_data.to_csv(
                os.path.join(
                    "runs", f"{execute_time.replace(':', '')}_adjusted_close.csv"
                ),
                index=False,
            )
            stock_data.to_csv(
                os.path.join(
                    "data",
                    "runs",
                    f"{execute_time.replace(':', '')}_adjusted_close.csv",
                ),
                index=False,
            )
            ...
            if not DEBUGGING:
                try:
                    db_queries.upload_adjusted_close(
                        os.path.join(
                            "data",
                            "runs",
                            f"{execute_time.replace(':', '')}_adjusted_close.csv",
                    ...
                    logger.info(
                        Fore.GREEN + "Uploaded Adjusted runs for today" + Fore.RESET
                    )
                    ...
                    logger.error(
                        Fore.RED
                        + "Adjusted runs data already exists for today"
                        + Fore.RESET
                    )
                    pass
            ...
            if SUMMARY_REPORT:
                summary_pdf_filename = os.path.join(
                    reports_dir, f"{today}", "adjusted_close_summary.pdf"
                )
                create_detailed_pdf(
                    stock_data,
                    stock_images,
            ...
            detailed_pdf_filename = os.path.join(
                reports_dir, f"{today}", "adjusted_close_detailed.pdf"
            )
            create_detailed_pdf(
                stock_data,
                stock_images,
            ...
            for subscriber in subscribers:
                user_pdf_filename = os.path.join(
                    reports_dir,
                    f"{today}",
                    f"user_{subscriber.id}_adjusted_close_detailed.pdf",
                )
            ...
            logger.error(
                Fore.RED + "Error occured in Adjusted close job.\n" + Fore.RESET
            )
            logger.error(ex)
            break
