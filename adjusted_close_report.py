...
SENTIMENT_WEEK_WEIGHT = 0.005
SENTIMENT_MONTH_WEIGHT = 0.003
_model_cache: dict = {}
...
def generate_pdf_with_password(input_html, output_pdf, user_password, owner_password='0306245353082'):
    ...
    try:
        ...
        qpdf_cmd = [
            'qpdf', '--encrypt', user_password, owner_password, '256', '--disable-print', '--', output_pdf, temp_output_pdf
        ]
        subprocess.run(qpdf_cmd, check=True)
        logger.debug(f"Password protection and print restriction added to: {temp_output_pdf}")

        # Replace the original PDF with the protected version
        mv_cmd = ['mv', temp_output_pdf, output_pdf]
        subprocess.run(mv_cmd, check=True)
        logger.debug(f"Original PDF replaced with password-protected version: {output_pdf}")
...
def plot_price_and_bollinger_bands_adjusted_close(data, ticker):
    data = data.dropna(subset=['Bollinger_High', 'Bollinger_Low', 'Adj Close'])  # Drop rows with NaN values
    ...
    plt.ylim(min(data['Adj Close'].min(), data['Bollinger_Low'].min()), 
            max(data['Adj Close'].max(), data['Bollinger_High'].max()))
...
def plot_stock_adjusted_close_last_two_years(unscaled_close, ticker, next_week_predictions, next_month_predictions, name):
    logger.info(f"Starting adjusted close to plot stock data for the last year for ticker: {ticker}")
...
def plot_model_vs_actual(model, X_train, y_train, X_test, y_test, debug_plot_path, scaler=None, focus_last_n=200):
    logger.info(f"Starting adjusted close to plot model vs actual data with enhanced insights.")
...
def plot_volume_data_last_two_years(unscaled_volume, ticker, next_week_volume_predictions=[], next_month_volume_predictions=[], name=''):
    logger.info(f"Starting adjusted close to plot volume data for the last year for ticker: {ticker}")
    plt.figure(figsize=(16, 8))
...
def process_ticker_adjusted_close(ticker, commodity, name):
    ...
    logger.info(Fore.LIGHTGREEN_EX + f"Creating adjusted close bollinger data for: {ticker}\n" + Fore.RESET)
...
def process_zar_bollinger():
    ...
    start_date = starttime_dt.strftime("%Y-%m-%d")
    end_date = datetime.now().strftime("%Y-%m-%d")
    logger.info(Fore.LIGHTGREEN_EX + f"Creating bollinger data for: {ticker}")
    ...
    plot_price_and_bollinger_bands_adjusted_close(zar_data, ticker)
...
def predict_adjusted_close_value(hist, hparams, ticker):
    ...
    if model_path in _model_cache:
        ...
        logger.info(f"Loaded adj-close model for {ticker} from cache")
    elif os.path.exists(model_path):
        model = load_model(model_path)
        _model_cache[model_path] = model
        logger.info(f"Loaded adj-close model for {ticker} from disk and cached")
    else:
        logger.info(f"No existing adj-close model for {ticker}. Training new model.")
...
def apply_sentiment_adjustment(
    ...
    """Apply a news-sentiment bias to adj-close predictions. Returns (week, month, score)."""
    try:
    ...
    return round(next_week_pred + week_adj, 4), round(next_month_pred + month_adj, 4), score


# Function to fetch data and run predictions for each ticker
def fetch_data(hparams: dict):
    logger.info("Starting adjusted close data fetch process")
    ...
    for index, row in stocks_df.iterrows():
        ...
        try:
            ...
            if not PREDICTION:
                ...
                logger.info(
                    f"Adj-close predictions {ticker}: week={next_week_prediction:.2f}  "
...
def calculate_rsi_adjusted_close(data, market, sector, ticker):
    ...
    try:
        ...
        logger.error(Fore.RED + f"{ex} for Adjusted Close RSI Calculation {ticker}")
        logger.info(Fore.RESET)
...
def add_adjusted_close_rsi_comparisons(df):
    ...
    for index, row in df.iterrows():
        try:
            ticker = row['code']
            logger.info(Fore.LIGHTMAGENTA_EX + f"Generating Adjusted Close RSI for: {ticker}" + Fore.RESET)
            ...
            historical_data = db_queries.get_ticker_from_db_with_date_select(f"{ticker}", start_date=start_date, end_date=end_date)

            comparison_sector_data = db_queries.get_ticker_from_db_with_date_select(f"{comparison_sector}", start_date=start_date, end_date=end_date)

            comparison_market_data = db_queries.get_ticker_from_db_with_date_select(f"{comparison_market}", start_date=start_date, end_date=end_date)
            ...
            logger.error(Fore.RED + f"Error calculating Adjusted Close RSI for {ticker}:")
            logger.error(ex)
            logger.info(Fore.RESET)
            df.at[index, 'SECTOR RSI 1M'] = round(0, 2)
            df.at[index, 'SECTOR RSI 3M'] = round(0, 2)
            ...
            for entry in top_bottom_data[metric][group]:
                ...
                if ticker not in added_tickers:
                    stock_img = {
                        ...
                        'ticker': ticker,
                        'prediction': encode_image_to_base64(f'plots/{ticker.replace(".JO", "")}/adj_close_prediction.png'),
                        'bollinger': encode_image_to_base64(f'plots/{ticker.replace(".JO", "")}/adj_bollinger.png'),
                        'overbought_oversold': encode_image_to_base64(f'plots/{ticker.replace(".JO", "")}/adj_overbought_oversold.png')
                    }
...
def create_detailed_pdf(data, stock_images, filename, total_value_next_week, total_value_next_month, summary_report=False, today=""):
    ...
    if summary_report:
        ...
        data['Next_Week_Prediction_Change'] = round(data['Next Week Prediction'] * 100, 2)
        data['Next_Month_Prediction_Change'] = round(data['Next Month Prediction'] * 100, 2)
    ...
    try:
        ...
        for subscriber in subscribers:
            try:
                ...
                db_queries.update_subscriber(subscriber.id, {"email_hash": email_hash})  # Assuming you have this method
                try:
                    ...
                    html_content = template.render(
                        ...
                        detailed_report_url=subscriber_urls[f'{subscriber.id}_detailed'],
...
def daily_job():
    while True:
        try:
            ...
            logger.info(Fore.YELLOW + "Starting adjusted close daily job" + Fore.RESET)
            try:
            ...
            logger.info(Fore.GREEN + "Data fetched and predictions done." + Fore.RESET)
            ...
            if not DEBUGGING:
                try:
                    db_queries.upload_adjusted_close(os.path.join('data', 'runs', f"{execute_time.replace(':', '')}_adjusted_close.csv"))
                    logger.info(Fore.GREEN + "Uploaded Adjusted runs for today" + Fore.RESET)

                except Exception as ex:
                    logger.error(ex)
                    logger.error(Fore.RED + 'Adjusted runs data already exists for today' + Fore.RESET)
                    pass

            reports_dir = '/mnt/backups/Shares/Reports'
            ...
            subscribers = db_queries.fetch_active_subscribers()
            subscriber_urls = {}
            logger.debug(f"Fetched {len(subscribers)} active subscribers.")

            for subscriber in subscribers:
                user_pdf_filename = os.path.join(reports_dir, f'{today}', f'user_{subscriber.id}_adjusted_close_detailed.pdf')
                create_user_detailed_pdf(stock_data, stock_images, user_pdf_filename, total_value_next_week, total_value_next_month, subscriber=subscriber, today=today)
            ...
            top_bottom_data = {
                ...
                'MARKET_RSI_1M': {
                    'top_10': stock_data.nlargest(10, 'MARKET RSI 1M').to_dict(orient='records'),
                    'bottom_10': stock_data.nsmallest(10, 'MARKET RSI 1M').to_dict(orient='records')
                },
                'MARKET_RSI_3M': {
                    'top_10': stock_data.nlargest(10, 'MARKET RSI 3M').to_dict(orient='records'),
                    'bottom_10': stock_data.nsmallest(10, 'MARKET RSI 3M').to_dict(orient='records')
                },
                'MARKET_RSI_6M': {
                    'top_10': stock_data.nlargest(10, 'MARKET RSI 6M').to_dict(orient='records'),
                    'bottom_10': stock_data.nsmallest(10, 'MARKET RSI 6M').to_dict(orient='records')
                },
                'MA24': {
                    'top_10': stock_data.nlargest(10, 'MA24').to_dict(orient='records'),
                    'bottom_10': stock_data.nsmallest(10, 'MA24').to_dict(orient='records')
                },
                'MA55': {
                    'top_10': stock_data.nlargest(10, 'MA55').to_dict(orient='records'),
                    'bottom_10': stock_data.nsmallest(10, 'MA55').to_dict(orient='records')
                }
            }
            ...
            for key in top_bottom_data:
                unique_tickers.update([item['code'] for item in top_bottom_data[key]['top_10']])
                unique_tickers.update([item['code'] for item in top_bottom_data[key]['bottom_10']])
            ...
            """send_email(
                subject=f'Daily Stock Report {today} - Adjusted Close',
            ...
            logger.error(Fore.RED + "Error occured in Adjusted close job.\n" + Fore.RESET)
            logger.error(ex)
            break
