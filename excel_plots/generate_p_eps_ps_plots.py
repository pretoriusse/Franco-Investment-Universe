...
def process_data(df):
    ...
    try:
        ...
        # Ensure 'P_EPS' exists
        p_sales_column = "P_EPS"
        if p_sales_column not in df.columns:
            # Attempt to find 'P_EPS' with different cases or slight variations
            possible_columns = [
                col for col in df.columns if col.lower() == "p_sales_ps".lower()
            ]
            if possible_columns:
                p_sales_column = possible_columns[0]
                df.rename(columns={p_sales_column: "P_EPS"}, inplace=True)
                print(f"Renamed column '{p_sales_column}' to 'P_EPS'.")
            else:
                raise ValueError("The DataFrame does not contain a 'P_EPS' column.")
        ...
        # Convert 'P_EPS' and 'P__change_24m' to numeric, coercing errors
        df["P_EPS"] = pd.to_numeric(df["P_EPS"], errors="coerce")
        df["P__change_24m"] = pd.to_numeric(df["P__change_24m"], errors="coerce")

        # Drop rows with NaN in 'P_EPS' or 'P__change_24m'
        initial_count = len(df)
        df = df.dropna(subset=["P_EPS", "P__change_24m"]).reset_index(drop=True)
        final_count = len(df)
        print(
            f"Dropped {initial_count - final_count} rows due to NaN in 'P_EPS' or 'P__change_24m'."
        )

        # Find the first non-zero 'P_EPS' and add one year to that date
        non_zero_eps_date = df[df["P_EPS"] > 0][date_column].min()
        if pd.isna(non_zero_eps_date):
            raise ValueError("No non-zero 'P_EPS' values found in the data.")
        ...
        # Remove data points where P_EPS <= 0 to eliminate clutter around 0
        before_filter = len(df)
        df = df[df["P_EPS"] > 0].reset_index(drop=True)
        after_filter = len(df)
        print(f"Removed {before_filter - after_filter} rows where 'P_EPS' <= 0.")
        ...
        df["Sub Set"] = df.groupby("Main Set")["P_EPS"].transform(
...
def generate_graph(
    ...
    try:
        ...
        # Add a vertical line at the latest P_EPS value
        ...
        plt.title(f"Scatter Plot of {title} vs P_EPS by Time Period", fontsize=18)
        plt.xlabel("P_EPS", fontsize=14)
        ...
        # Save the plot
        save_path = os.path.join(save_dir, "P_EPS_Scatter_Plot.png")
...
def main():
    ...
    x_column = "P_EPS"  # Independent variable
    y_column = "P__change_24m"  # Dependent variable
    title = "P_EPS"  # Title for the x-axis
