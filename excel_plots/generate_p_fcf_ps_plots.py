...
def process_data(df):
    ...
    try:
        ...
        # Ensure 'P_FCF_ps' exists
        p_sales_column = "P_FCF_ps"
        if p_sales_column not in df.columns:
            # Attempt to find 'P_FCF_ps' with different cases or slight variations
            possible_columns = [
                col for col in df.columns if col.lower() == "p_sales_ps".lower()
            ]
            if possible_columns:
                p_sales_column = possible_columns[0]
                df.rename(columns={p_sales_column: "P_FCF_ps"}, inplace=True)
                print(f"Renamed column '{p_sales_column}' to 'P_FCF_ps'.")
            else:
                raise ValueError("The DataFrame does not contain a 'P_FCF_ps' column.")
        ...
        # Convert 'P_FCF_ps' and 'P__change_24m' to numeric, coercing errors
        df["P_FCF_ps"] = pd.to_numeric(df["P_FCF_ps"], errors="coerce")
        df["P__change_24m"] = pd.to_numeric(df["P__change_24m"], errors="coerce")

        # Drop rows with NaN in 'P_FCF_ps' or 'P__change_24m'
        initial_count = len(df)
        df = df.dropna(subset=["P_FCF_ps", "P__change_24m"]).reset_index(drop=True)
        final_count = len(df)
        print(
            f"Dropped {initial_count - final_count} rows due to NaN in 'P_FCF_ps' or 'P__change_24m'."
        )

        # Find the first non-zero 'P_FCF_ps' and add one year to that date
        non_zero_eps_date = df[df["P_FCF_ps"] > 0][date_column].min()
        if pd.isna(non_zero_eps_date):
            raise ValueError("No non-zero 'P_FCF_ps' values found in the data.")
        ...
        # Remove data points where P_FCF_ps <= 0 to eliminate clutter around 0
        before_filter = len(df)
        df = df[df["P_FCF_ps"] > 0].reset_index(drop=True)
        after_filter = len(df)
        print(f"Removed {before_filter - after_filter} rows where 'P_FCF_ps' <= 0.")
        ...
        df["Sub Set"] = df.groupby("Main Set")["P_FCF_ps"].transform(
...
def generate_graph(
    ...
    try:
        ...
        # Add a vertical line at the latest P_FCF_ps value
        ...
        plt.title(f"Scatter Plot of {title} vs P_FCF_ps by Time Period", fontsize=18)
        plt.xlabel("P_FCF_ps", fontsize=14)
        ...
        # Save the plot
        save_path = os.path.join(save_dir, "P_FCF_ps_Scatter_Plot.png")
...
def main():
    ...
    x_column = "P_FCF_ps"  # Independent variable
    y_column = "P__change_24m"  # Dependent variable
    title = "P_FCF_ps"  # Title for the x-axis
